% Modified by Patrick Leo August 14th 2018 to only open one specific layer
% of an image
% Added layer input
function [result] = bfopenSpecificLayer(id, layer, varargin)
...
if nargin == 0 || (isstring(id) && exist(id, 'file') == 0)
...
% If we were given a string rather than a reader object
if(isstring(id))
...
for i = 1:numImages
    ...
    % Don't retrieve color map data - Patrick 2018
%     if bpp == 1
...
%     warning (warning_state);
    ...
    % Don't build an informative title - Patrick, Sep. 2018
%     label = id;
%     if numSeries > 1
...
% Don't extract metadata - Patrick Sep. 2018
